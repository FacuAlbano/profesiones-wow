...
import { MIDNIGHT_SUBIDA_MATERIALS } from "~/lib/midnight-subida-materials";
import { MIDNIGHT_SUBIDA_NPCS } from "~/lib/midnight-subida-npcs";
...
function WowheadItem(
  ...
  return (
    <WowheadItemBase
      materials={MIDNIGHT_SUBIDA_MATERIALS}
      game="midnight"
      {...props}
    />
  );
...
function WowheadNpc(props: Omit<ComponentProps<typeof WowheadNpcBase>, "npcs" | "game">) {
  return <WowheadNpcBase npcs={MIDNIGHT_SUBIDA_NPCS} game="midnight" {...props} />;
}
    ...
    if (kind === "r" || kind === "h") {
      ...
      nodes.push(
        <WowheadEntityLink
          ...
          entity={{ tipo, id: Number(id), juego: "midnight" }}
        >
          {name}
        </WowheadEntityLink>,
      );
...
export function midnightSubidaIndex(spec: MidnightSubidaSpec): GuideIndexSection[] {
  return [
...
export function MidnightSubidaGuide({ spec }: { spec: MidnightSubidaSpec }) {
  return (
    <div className="space-y-8 animate-in fade-in duration-300">
      ...
      <section id="cruces" className="scroll-mt-24">
        ...
        <ul className="mt-3 list-inside list-disc space-y-1 text-sm text-card-foreground">
          <li>
            <Link
              to={`/expansion/midnight/profesion/${spec.slug}/recetas-y-conocimiento`}
          ...
          <li>
            <Link
              to={`/expansion/midnight/profesion/${spec.slug}/especializaciones`}
              className="link-faction"
            >
              Especializaciones
            </Link>
          ...
          <li>
            <Link
              to={`/expansion/midnight/profesion/${spec.slug}/farming`}
              className="link-faction"
            >
              Farming
            </Link>
          ...
          <li>
            <Link
              to={`/expansion/midnight/profesion/${spec.pairing.slug}`}
              className="link-faction"
            >
              {spec.pairing.name}
            </Link>
