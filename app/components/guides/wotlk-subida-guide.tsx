...
import { WOTLK_SUBIDA_MATERIALS } from "~/lib/wotlk-subida-materials";
import { WOTLK_SUBIDA_NPCS } from "~/lib/wotlk-subida-npcs";
...
function WowheadItem(
  ...
  return <WowheadItemBase materials={WOTLK_SUBIDA_MATERIALS} game="wotlk" {...props} />;
}

function WowheadNpc(props: Omit<ComponentProps<typeof WowheadNpcBase>, "npcs" | "game">) {
  return <WowheadNpcBase npcs={WOTLK_SUBIDA_NPCS} game="wotlk" {...props} />;
}
    ...
    if (kind === "r" || kind === "h") {
      ...
      nodes.push(
        <WowheadEntityLink
          ...
          entity={{ tipo, id: Number(id), juego: "wotlk" }}
        >
          {name}
        </WowheadEntityLink>,
      );
...
const FACTIONS = [
  { id: "horda", label: "Horda", trainer: "trainerHordeKey", vendor: "vendorHordeKey" },
  { id: "alianza", label: "Alianza", trainer: "trainerAllianceKey", vendor: "vendorAllianceKey" },
] as const;

export function wotlkSubidaIndex(spec: WotlkSubidaSpec): GuideIndexSection[] {
  return [
    { id: "intro", label: "Introducción" },
    {
      id: "instructores",
      label:
        spec.vendorHordeKey || spec.vendorAllianceKey ? "Entrenador y vendedor" : "Entrenador",
    },
...
export function WotlkSubidaGuide({ spec }: { spec: WotlkSubidaSpec }) {
  const hasVendor = Boolean(spec.vendorHordeKey || spec.vendorAllianceKey);
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-300">
      ...
      <section id="instructores" className="scroll-mt-24">
        <h2 className="guide-division font-heading text-xl font-semibold sm:text-2xl">
          {hasVendor ? "Entrenador y vendedor" : "Entrenador"}
        </h2>
        <p className="mt-2 text-sm text-muted-foreground">{renderRichText(spec.trainerNote)}</p>
        <div className="mt-4 grid gap-4 sm:grid-cols-2">
          {FACTIONS.map((faction) => {
            const trainerKey = spec[faction.trainer];
            const vendorKey = spec[faction.vendor];
            return (
              <Card key={faction.id} className="card-faction border border-border">
                <CardContent className="p-4">
                  ...
                  <ul className="mt-2 space-y-1 text-sm text-card-foreground">
                    <li>
                      Entrenador: <WowheadNpc npcKey={trainerKey} />
                    </li>
                    {vendorKey ? (
                      <li>
                        Vendedor: <WowheadNpc npcKey={vendorKey} />
                      </li>
                    ) : null}
      ...
      <section id="cruces" className="scroll-mt-24">
        ...
        <ul className="mt-3 list-inside list-disc space-y-1 text-sm text-card-foreground">
          <li>
            <Link
              to={`/expansion/wrath-of-the-lich-king/profesion/${spec.slug}/recetas-y-conocimiento`}
          ...
          <li>
            <Link
              to={`/expansion/wrath-of-the-lich-king/profesion/${spec.slug}/especializaciones`}
              className="link-faction"
            >
              Especializaciones
            </Link>
          ...
          <li>
            <Link
              to={`/expansion/wrath-of-the-lich-king/profesion/${spec.slug}/farming`}
              className="link-faction"
            >
              Farming
            </Link>
          ...
          <li>
            <Link
              to={`/expansion/wrath-of-the-lich-king/profesion/${spec.pairing.slug}`}
              className="link-faction"
            >
              {spec.pairing.name}
            </Link>
