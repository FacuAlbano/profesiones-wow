...
import type { CatalogSubidaBundle, CatalogSubidaSpec } from "~/lib/catalog-subida-types";
...
export function catalogSubidaIndex(spec: CatalogSubidaSpec): GuideIndexSection[] {
  return [
...
export function CatalogSubidaGuide({ bundle }: { bundle: CatalogSubidaBundle }) {
  const { spec, materials, npcs, game, expansionSlug } = bundle;
  ...
  function WowheadItem(
    ...
    return <WowheadItemBase materials={materials} game={game} {...props} />;
  }

  function WowheadNpc(props: Omit<ComponentProps<typeof WowheadNpcBase>, "npcs" | "game">) {
    return <WowheadNpcBase npcs={npcs} game={game} {...props} />;
  }
      ...
      if (kind === "r" || kind === "h") {
        ...
        nodes.push(
          <WowheadEntityLink
            ...
            entity={{ tipo, id: Number(id), juego: game }}
          >
            {name}
          </WowheadEntityLink>,
        );
    ...
    <div className="space-y-8 animate-in fade-in duration-300">
      ...
      <section id="cruces" className="scroll-mt-24">
        ...
        <ul className="mt-3 list-inside list-disc space-y-1 text-sm text-card-foreground">
          <li>
            <Link
              to={`/expansion/${expansionSlug}/profesion/${spec.slug}/recetas-y-conocimiento`}
          ...
          <li>
            <Link
              to={`/expansion/${expansionSlug}/profesion/${spec.slug}/especializaciones`}
              className="link-faction"
            >
              Especializaciones
            </Link>
          ...
          <li>
            <Link
              to={`/expansion/${expansionSlug}/profesion/${spec.slug}/farming`}
              className="link-faction"
            >
              Farming
            </Link>
          ...
          <li>
            <Link
              to={`/expansion/${expansionSlug}/profesion/${spec.pairing.slug}`}
              className="link-faction"
            >
              {spec.pairing.name}
            </Link>
