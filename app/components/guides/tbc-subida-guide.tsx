...
import { TBC_SUBIDA_NPCS } from "~/lib/tbc-subida-npcs";
...
function WowheadItem(
  ...
  return <WowheadItemBase materials={TBC_SUBIDA_MATERIALS} game="tbc" {...props} />;
}

function WowheadNpc(props: Omit<ComponentProps<typeof WowheadNpcBase>, "npcs" | "game">) {
  return <WowheadNpcBase npcs={TBC_SUBIDA_NPCS} game="tbc" {...props} />;
}
    ...
    if (kind === "r" || kind === "h") {
      ...
      nodes.push(
        <WowheadEntityLink
          ...
          entity={{ tipo, id: Number(id), juego: "tbc" }}
        >
          {name}
        </WowheadEntityLink>,
      );
...
export function tbcSubidaIndex(spec: TbcSubidaSpec): GuideIndexSection[] {
  return [
...
export function TbcSubidaGuide({ spec }: { spec: TbcSubidaSpec }) {
    ...
    <div className="space-y-8 animate-in fade-in duration-300">
      ...
      <section id="cruces" className="scroll-mt-24">
        ...
        <ul className="mt-3 list-inside list-disc space-y-1 text-sm text-card-foreground">
          <li>
            <Link
              to={`/expansion/the-burning-crusade/profesion/${spec.slug}/recetas-y-conocimiento`}
          ...
          <li>
            <Link
              to={`/expansion/the-burning-crusade/profesion/${spec.slug}/especializaciones`}
              className="link-faction"
            >
              Especializaciones
            </Link>
          ...
          <li>
            <Link
              to={`/expansion/the-burning-crusade/profesion/${spec.slug}/farming`}
              className="link-faction"
            >
              Farming
            </Link>
          ...
          <li>
            <Link
              to={`/expansion/the-burning-crusade/profesion/${spec.pairing.slug}`}
              className="link-faction"
            >
              {spec.pairing.name}
            </Link>
